...
export default function App() {
  ...
  return <div className="relative flex h-full min-h-[560px] flex-col overflow-hidden bg-[var(--space-surface-bg)] text-[var(--space-text-primary)]">
    <header className="flex h-16 shrink-0 items-center gap-3 border-b border-[var(--space-border-default)] bg-[var(--space-surface-card)] px-5"><button onClick={() => { setView('library'); setProject(null); void refreshLibrary(); }} className="flex items-center gap-2 font-bold"><span className="flex h-9 w-9 items-center justify-center rounded-xl bg-[var(--space-brand-primary-600)]"><Film className="h-5 w-5 text-white" /></span>SceneForge</button><span className="hidden text-sm text-[var(--space-text-muted)] sm:block">Avatar-first explainers, directed scene by scene</span><div className="ml-auto flex gap-2"><button onClick={() => setVergerOpen(true)} className="flex items-center gap-2 rounded-xl border border-[var(--space-border-default)] px-3 py-2 text-sm"><MessageSquareText className="h-4 w-4" />Verger</button><button onClick={() => setView('settings')} className="rounded-xl border border-[var(--space-border-default)] p-2"><Settings className="h-4 w-4" /></button></div></header>
    {error ? <div className="flex items-center justify-between bg-[color-mix(in_srgb,var(--space-semantic-danger-500)_14%,transparent)] px-5 py-2 text-sm text-[var(--space-semantic-danger)]"><span>{error}</span><button onClick={() => setError('')}><X className="h-4 w-4" /></button></div> : null}
    ...
    {vergerOpen ? <aside className="absolute inset-y-0 right-0 z-20 flex w-full max-w-md flex-col border-l border-[var(--space-border-default)] bg-[var(--space-surface-card)] p-5 shadow-2xl"><div className="flex items-center justify-between"><div><h2 className="font-bold">Verger</h2><p className="text-xs text-[var(--space-text-muted)]">Coordinator · routes only what you ask</p></div><button onClick={() => setVergerOpen(false)}><X className="h-5 w-5" /></button></div><div className="mt-6 flex-1 rounded-2xl bg-[var(--space-surface-panel)] p-4 text-sm text-[var(--space-text-secondary)]">{vergerReply || 'Try “make scene 3 a pie chart”, “redo the script intro”, or “switch everything to documentary”. Active scene edits are queued, never overwritten.'}</div><textarea value={vergerText} onChange={(e) => setVergerText(e.target.value)} rows={4} placeholder="Tell Verger what to change…" className="mt-4 rounded-xl border border-[var(--space-border-default)] bg-[var(--space-surface-panel)] p-3 text-[var(--space-text-primary)]" /><button disabled={busy || !project || !vergerText.trim()} onClick={() => void askVerger()} className="mt-3 flex items-center justify-center gap-2 rounded-xl bg-[var(--space-brand-primary-600)] px-4 py-3 font-semibold text-white disabled:opacity-40">{busy ? <Loader2 className="h-4 w-4 animate-spin" /> : <Send className="h-4 w-4" />}Send to Verger</button></aside> : null}
  </div>;
}
...
function Library({ projects, loading, onOpen, onNew }: { projects: Project[]; loading: boolean; onOpen: (id: string) => void; onNew: () => void }) { return <div className="mx-auto max-w-6xl px-6 py-10"><div className="flex items-end justify-between"><div><p className="text-xs font-bold uppercase tracking-[.2em] text-[var(--space-text-brand)]">Your studio</p><h1 className="mt-2 text-3xl font-bold">SceneForge projects</h1></div><button onClick={onNew} className="flex items-center gap-2 rounded-xl bg-[var(--space-brand-primary-600)] px-4 py-3 font-semibold text-white"><Plus className="h-4 w-4" />New project</button></div>{loading ? <Loader2 className="mt-8 h-5 w-5 animate-spin" /> : <div className="mt-7 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{projects.map((project) => <button key={project.id} onClick={() => onOpen(project.id)} className="rounded-2xl border border-[var(--space-border-default)] bg-[var(--space-surface-card)] p-5 text-left"><FolderOpen className="h-5 w-5 text-[var(--space-text-brand)]" /><div className="mt-4 line-clamp-2 font-semibold">{project.topic}</div><div className="mt-2 text-xs uppercase tracking-wide text-[var(--space-text-muted)]">{project.status.replace(/_/g, ' ')} · {project.aspect_ratio}</div></button>)}{!projects.length ? <div className="col-span-full rounded-2xl border border-dashed border-[var(--space-border-default)] p-12 text-center text-[var(--space-text-muted)]">No v2 projects yet. Start with a topic.</div> : null}</div>}</div>; }
