...
// Customer-tools registry entries reach the chat namespaced by the MCP server
// that serves them (e.g. `mcp__otto-customer-tools__produce_video`), so strip
// that wrapper before looking the tool up.
const TOOL_NAME_PREFIX_RE = /^(?:functions\.|tools\.|mcp__.+?__)/;
...
const toolTerms: Record<string, string> = {
  ...
  // VidVerge video production (customer-tools registry)
  'produce_video': 'sending your video to the render queue',
  'check_video_status': 'checking on your render',
  'ToolSearch': 'getting the studio ready',
...
const toolColors: Record<string, string> = {
  ...
  // VidVerge video production
  'produce_video': 'text-pink-600',
  'check_video_status': 'text-sky-600',
  'ToolSearch': 'text-gray-500',
