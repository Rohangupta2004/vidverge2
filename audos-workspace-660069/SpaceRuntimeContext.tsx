...
} from './lib/tenant-delegation-scope';
...
/**
 * Workspace data APIs only accept canonical server sessions (wses_) or the
 * explicit guest flow (guest_). Local storage is merely a remembered proposal:
 * the platform sets __audosAcceptedSessionId only after validating it for this
 * tab. Never let a stale/client-invented id suppress EmailGate and render apps
 * without an authenticated WorkspaceDB session.
 */
function canonicalSessionId(value: unknown): string | undefined {
  if (typeof value !== 'string') return undefined;
  return value.startsWith('wses_') || value.startsWith('guest_') ? value : undefined;
}

function acceptedStoredSessionId(value: unknown): string | undefined {
  const candidate = canonicalSessionId(value);
  if (!candidate || typeof window === 'undefined') return undefined;
  return (window as any).__audosAcceptedSessionId === candidate ? candidate : undefined;
}

function acceptFreshSessionId(value: unknown): string | undefined {
  const candidate = canonicalSessionId(value);
  if (!candidate || typeof window === 'undefined') return undefined;
  ...
  runtimeWindow.__audosAcceptedSessionId = candidate;
  runtimeWindow.__workspaceDb?.setSessionId?.(candidate);
  return candidate;
}
...
export function SpaceRuntimeProvider({ 
  ...
  const [sessionId, setSessionIdRaw] = useState<string | undefined>(() => {
    ...
    if (initialSessionId) {
      // A provider prop was resolved by the server for this page load.
      resolved = canonicalSessionId(initialSessionId);
    } else {
      try {
        // Product Run serve-time bootstrap may have seeded wses_ on __TENANT_DELEGATION__.
        const delegation = getTenantDelegationScope();
        if (delegation?.workspaceSessionId) {
          resolved = canonicalSessionId(delegation.workspaceSessionId);
        }
        if (!resolved) {
          ...
          if (stored) {
            ...
            resolved = acceptedStoredSessionId(recoveredId);
          }
        }
        if (!resolved) {
          resolved = acceptedStoredSessionId(sessionStorage.getItem('space_session_id'));
        }
      } catch {
        // Storage can be unavailable or malformed. Stay unresolved so the
        // authentication gate renders; a fabricated id would make every data
        // function fail after the shell incorrectly treated it as signed in.
        resolved = undefined;
      }
    }
    // Under delegation with no session yet: stay undefined so bootstrap can complete
    // (an ephemeral id would suppress the gate and skip the real wses_ path).
    if (
      !resolved &&
      typeof window !== 'undefined' &&
      hasTenantDelegationAuth()
  ...
  // True while post-checkout or tenant-delegation auto-session bootstrap is in flight
  const [isBootstrappingSession, setIsBootstrappingSession] = useState<boolean>(() => {
    ...
    const delegation = getTenantDelegationScope();
    if (
      delegation?.workspaceSessionId &&
      typeof delegation.workspaceSessionId === 'string' &&
      delegation.workspaceSessionId.startsWith('wses_')
    ) {
      return false;
    }
    if (hasTenantDelegationAuth()) return true;
  ...
  const setSessionId = useCallback((newSessionId: string) => {
    const safeSessionId = acceptedStoredSessionId(newSessionId);
    if (!safeSessionId) {
      console.warn('[SpaceRuntime] Ignoring session ID that was not accepted for this tab');
      return;
    }
    setSessionIdRaw(safeSessionId);
    if (typeof window !== 'undefined') {
      (window as any).__spaceSessionId = safeSessionId;
    }
    try {
      const stored = localStorage.getItem(scopedSpaceSessionStorageKey(spaceId));
      if (stored) {
  ...
  useEffect(() => {
    ...
    const autoCreateSession = async () => {
      try {
        ...
        acceptFreshSessionId(effectiveSessionId);

        try {
          window.dispatchEvent(new CustomEvent('audos:session-established', {
            detail: { workspaceSessionId: effectiveSessionId, email: customerEmail, verified: true },
          }));
        } catch {}
  ...
  // Tenant-agent Product Run: bootstrap a consumer-scoped wses_ when serve-time
  // injection did not seed one (or React init missed the seeded localStorage).
  useEffect(() => {
    if (mode !== 'customer') return;
    if (!hasTenantDelegationAuth()) return;
    ...
    // Prefer already-seeded session from head injection / localStorage
    try {
      const injectedId = delegation?.workspaceSessionId;
      if (injectedId && injectedId.startsWith('wses_')) {
        const sessionKey = scopedSpaceSessionStorageKey(spaceId);
        const existing = localStorage.getItem(sessionKey);
        if (!existing && delegation?.email) {
          localStorage.setItem(
            sessionKey,
            JSON.stringify({
              id: injectedId,
              workspaceSessionId: injectedId,
        ...
        acceptFreshSessionId(injectedId);
      ...
      if (stored) {
        const session = JSON.parse(stored);
        const recoveredId = session.workspaceSessionId || session.sessionId || session.id;
        if (recoveredId && typeof recoveredId === 'string' && recoveredId.startsWith('wses_')) {
          if (!sessionId || !sessionId.startsWith('wses_')) {
    ...
    const bootstrapDelegationSession = async () => {
      try {
        console.log('[SpaceRuntime] Tenant delegation detected — bootstrapping consumer session');
        const res = await fetch(`/api/space/${spaceId}/delegation-session-bootstrap`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          body: JSON.stringify({
        ...
        if (!res.ok) {
          console.warn('[SpaceRuntime] Delegation session bootstrap failed:', res.status);
          setIsBootstrappingSession(false);
          return;
        }
        const data = await res.json();
        const effectiveSessionId = data.workspaceSessionId;
        if (!effectiveSessionId || typeof effectiveSessionId !== 'string') {
          console.warn('[SpaceRuntime] Delegation bootstrap returned no session ID');
          setIsBootstrappingSession(false);
          return;
        }
        ...
        console.log('[SpaceRuntime] Delegation session established:', effectiveSessionId);
        setSessionId(effectiveSessionId);
        setIsBootstrappingSession(false);
      } catch (err) {
        console.error('[SpaceRuntime] Delegation session bootstrap failed:', err);
        setIsBootstrappingSession(false);
      }
    };
  ...
  const resolveSessionId = (): string | undefined => {
    ...
    try {
      ...
      if (stored) {
        const session = JSON.parse(stored);
        const recoveredId = acceptedStoredSessionId(
      ...
      const ssId = acceptedStoredSessionId(sessionStorage.getItem('space_session_id'));
      if (ssId) {
        setSessionId(ssId);
        return ssId;
      }
    } catch {
      // Stay unresolved. The shell must show authentication instead of
      // inventing an id the server and WorkspaceDB runtime will reject.
    }
    return undefined;
  };
  ...
  const checkAppAccess = useCallback((requiredTier?: string): boolean => {
    ...
    // Every caller is granted regardless of subscription state, so no feature
    // can be locked behind a tier and no upgrade prompt can be triggered from
    // an access check. The original gate is preserved untouched below — set
    // GATE_DISABLED to false (and flip SUBSCRIPTION_GATE_DISABLED in
    // lib/plans.ts) to restore the subscription checks.
  ...
  const firePurchaseEventWithRetry = async (email: string | null, stripeSessionId?: string | null) => {
    ...
    try {
      ...
      const qualifiedCurrency = typeof data.currency === 'string' && data.currency
        ? data.currency
        : 'usd';
      currency = qualifiedCurrency;
      console.log(`[SpaceRuntime] Purchase qualified: ${amount} ${qualifiedCurrency.toUpperCase()} (mode: ${data.mode}, paymentStatus: ${data.paymentStatus})`);
